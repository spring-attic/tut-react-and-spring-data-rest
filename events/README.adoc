...
:prev_session: https://github.com/gregturn/react-and-spring-data-rest/tree/master/conditional
...
In this session you'll learn how to use Spring Data REST's built in event system and publish updates to ALL users through Spring's WebSocket support. Then you'll be able to dynamically adjust clients as the data updates.

== Step 0 - Setting up your environment

To get off the ground, you can either https://github.com/gregturn/react-and-spring-data-rest/tree/master/events[grab the code] from this repository, or you can do it yourself, by hand. This project is based on the previous session with more modifications coming.

== Configuring WebSockets with Spring

http://docs.spring.io/spring/docs/current/spring-framework-reference/htmlsingle/#websocket[Spring comes with powerful WebSocket support]. One thing to recognize is that WebSocket is a very low level protocol. It doesn't little more than offer the means to transmit data between client and server. The recommendation is to use a sub-protocol (STOMP in this cases) to actually encode data and routes.
...
* `@EnableWebSocketMessageBroker` turns on the WebSocket support.
* `AbstractWebSocketMessageBrokerConfigurer` provides a convenient base class to configure basic features.
* *MESSAGE_PREFIX* is the prefix you will append to every message's route.
* `registerStompEndpoints()` is used to configure the endpoint on the server for the server and clients to connect (`/payroll`).
...
* `@RepositoryEventHandler(Employee.class)` flags this class as only applying to events on *employees*.
* A `SimpMessagingTemplate` and an `EntityLinks` are autowired from the application context.
* The `@HandlerZZZ` annotations are used to flag the methods that need to listen to. These methods must be public.

Each of these handler methods invokes the `SimpMessagingTemplate.convertAndSend()` to transmit a message over the WebSocket. This is a pub-sub approach so that one message is relayed to every attached consumer.
...
`getPath()` uses Spring Data REST's `EntityLinks` to look up the path for a given class type and id. To serve the client's needs, this `Link` object is converted to a Java URI and its path extracted.
...
Before we finish this section, there is soemthing to recognize. We just added a new way for the state in the UI to get updated: when a WebSocket message arrives. But the old way to update the state is still there.

To simplify our code's management of state, it's best to remove the old way. In other words, submit *POST*, *PUT*, and *DELETE* calls, but don't use their results to update the UI's state. Instead, wait for the WebSocket event and let the code up above do its thing.
...
Here, the `follow()` function is used to get to the *employees* link, and then the *POST* operation is applied. But the event handler to listen for when this op is done has effectively been moved `refreshAndGoToLastPage()` which you just looked at.
...
Paging is useful, but offers a tricky state to manage. The costs are low on this sample app, and React only sends a diff in state to DOM, so if no change occurred, there will be no flickering of the UI. 

But when designing with paging in mind, you have to decide what is the expected behavior between clients and if there needs to updates or not. Depending on your requirements and performance of the system, the existing navigational hypermedia may be sufficent.