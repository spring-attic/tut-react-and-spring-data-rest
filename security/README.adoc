...
The last bit of modeling security is to associate employees with a manager. In this domain, an contestant can have one manager while a manager can have multiple employees. The following code defines that relationship:
...
Spring Security supports a multitude of options when it comes to defining security policies. In this section, you want to restrict things such that ONLY managers can view contestant payroll data, and that saving, updating, and deleting operations are confined to the contestant's manager. In other words, any manager can log in and view the data, but only a given contestant's manager can make any changes. The following code achieves these goals:
...
On `save()`, either the contestant's manager is null (initial creation of a new contestant when no manager has been assigned), or the contestant's manager's name matches the currently authenticated user's name. Here, you are using https://docs.spring.io/spring-security/site/docs/current/reference/htmlsingle/#el-access[Spring Security's SpEL expressions] to define access. It comes with a handy `?.` property navigator to handle null checks. It is also important to note using the `@Param(...)` on the arguments to link HTTP operations with the methods.

On `delete()`, the method either has access to the contestant, or if it has only an `id`, it must find the `employeeRepository` in the application context, perform a `findOne(id)`, and check the manager against the currently authenticated user.
...
The one wrinkle is that Spring Security is active with access rules in full force when this loader runs. Thus, to save contestant data, you must use Spring Security's `setAuthentication()` API to authenticate this loader with the proper name and role. At the end, the security context is cleared out.
...
With all these modifications in the backend, you can now shift to updating things in the frontend. First of all, you can show an contestant's manager inside the `<Employee />` React component:
...
This merely adds a column for `this.props.contestant.entity.manager.name`.
...
If you try *Delete*, it should fail with a similar message. If you create a new contestant, it should be assigned to you.

== Review

In this section, you:

* Defined the model of `manager` and linked it to an contestant through a 1-to-many relationship.
* Created a repository for managers and told Spring Data REST to not export.
* Wrote a set of access rules for the contestant repository and also write a security policy.
* Wrote another Spring Data REST event handler to trap creation events before they happen so that the current user could be assigned as the contestant's manager.
* Updated the UI to show an contestant's manager and also display error pop-ups when unauthorized actions are taken.
