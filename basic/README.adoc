...
:next_session: https://github.com/gregturn/react-and-spring-data-rest/tree/master/hypermedia
...
To get off the ground, you can either https://github.com/gregturn/react-and-spring-data-rest/tree/master/basic[grab the code] from this repository, or you can do it yourself, by hand.
...
* Pick on some well accepted, industry standard protocols, like HTTP verbs, formats, standardized media types, and IANA-approved link names.
...
To start things off, it's very handy to have a top level container for all components. (This will become more evident as we expand upon the code throughout this series.) Right now, you only have the employee list. But you might need some other related components later on, so let's start with this:
...
These are things we can address in the {next_session}[next session]. Until then, happy coding!
