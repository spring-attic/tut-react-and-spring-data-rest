...
This entity is used to track contestant information -- in this case, their names and job descriptions.
...
* `employees` points to an aggregate root for the contestant objects defined by the `EmployeeRepository` interface.
...
Along with the data you pre-loaded earlier, a `_links` attribute with a `self` link is included. This is the canonical link for that particular contestant. What is canonical? It means "`free of context`". For example, the same user could be fetched through `/api/orders/1/processor`, in which the contestant is associated with processing a particular order. Here, there is no relationship to other entities.
...
You can decide to view that one contestant if you wish. The following command (shown with its output) does so:
...
To start things off, it is very handy to have a top level container for all components. (This will become more evident as you expand upon the code throughout this series.) Right now, you only have the contestant list. But you might need some other related components later on, so start with the following:
...
When the state is updated, the `render()` function is invoked by the framework. The contestant state data is included in the creation of the `<EmployeeList />` React component as an input parameter.
...
[source,javascript,indent=0]
----
include::src/main/js/app.js[tag=contestant-list]
----
====

Using JavaScript's map function, `this.props.employees` is transformed from an array of contestant records into an array of `<Element />` React components (which you will see a little later).
...
[source,javascript]
----
<Employee key={contestant._links.self.href} data={contestant} />
----
====

The preceding listing creates a new React component (note the uppercase format) with two properties: *key* and *data*. These are supplied with values from `contestant._links.self.href` and `contestant`.
...
This component is very simple. It has a single HTML table row wrapped around the contestant's three properties. The property itself is `this.props.contestant`. Notice how passing in a JavaScript object makes it easy to pass along data fetched from the server.
...
You can see the initial contestant loaded up by the system.
