...
----
$ curl -X POST localhost:8080/employees -d '{"firstName": "Bilbo", "lastName": "Baggins", "description": "burglar"}' -H 'Content-Type:application/json'
{
  "firstName" : "Bilbo",
  "lastName" : "Baggins",
  "description" : "burglar",
...
This application uses http://requirejs.org/[requirejs] to load modules at runtime. This tutorial won't go into extensive detail on JavaScript loading. Feel free to use whatever you prefer.

The follow bower modules are being used:

* requirejs
* rest
* jsx-requirejs-plugin (which pulls in react)

The paths for all your modules are defined in `run.js`:

.src/main/resources/static/run.js
[source,javascript]
----
include::src/main/resources/static/run.js[]
----

The `jsx` parts are used to flag JSX files and how they are handled.

Next, you need to load the React bits when the DOM finishes loading. 

.src/main/resources/static/main.js
[source,javascript]
----
include::src/main/resources/static/main.js[]
----

If you are using tools like jQuery, they have alternative ways to detect when the browser is done loading the DOM.

Now, for the gold, `app.jsx`. This contains our component definitions as well as code to fetch data from the backend. Let's look at in parts.

For starters, since you using requireJS to load things, go ahead and fetch the modules you need:
...
* `client` is a restJS configuration that includes suport for HAL, URI Template, and other things. We won't show it here.
...
Below is the defintion for an `EmployeeList`.
...
* `getInitialState` is the API to initialize data that is expected to vary.
...
In this component, an array of employees is fetched from the Spring Data REST back end stored in this component's *state* data.

[[NOTE]]
====
React has two types of data: *state* and *properties*. 

State is data that the component is expected to handle itself. It is also data that can fluctuate and change. To read the state, you use `this.state`. To update it, you use `this.setState()`. The common convention is to initialize state with nothing and declare all the parts in `getInitialState` and then load it from the server using `componentDidMount`. From there on, updates can be driven by user action or other events.
...
In this code, the function loads data via `client`, a https://promisesaplus.com/[Promise compliant] solution. When it is done retrieving from `/employees`, it then invokes the function inside `done()` and set's the state based on it's HAL document (`response.entity._embedded.employees`). You might remember the structure of `curl /employees` earlier and how it had *employees* as an array nested inside *_embedded*.

Further down in the `render()` method, `this.state.employees` is being mapped over to transform the array of employee records into an array of `<Employee/>` React components (which we'll define a little further down). It's important to note the creation of a new React component:
...
Here you are invoked a new component (uppercased) along with two properties: *key* and *data*. These are supplied the values from `employee._links.self.href` and `employee`.

IMPORTANT: Whenever you work with Spring Data REST, the *self* link IS the key for a given resource. React needs a unique identifer for lists of children, and `_links.self.href` is perfect.
...
In this session, we defined a domain object and a corresponding repository. Then we let Spring Data REST export it with full blown hypermedia controls. Next we created a front end using React. We declared some simple components and loaded them using requireJS. The front end fetched data from the back end and served it up as an unstructured list.

Issues?

* The web page wasn't dynamic. You had to refresh the browser to fetch new records.
* The web page didn't use any of the hypermedia controls. Instead, it was hardcoded to fetch data from `/employees`.
* It's read only. While you can alter records using cURL, the web page offers none of that.
* The page had no CSS styling.

These are issues we can tackle in the next session.