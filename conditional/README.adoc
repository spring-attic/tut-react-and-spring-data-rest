...
In the {prev_session}[previous session] you used the collection resource to gather data and populate the UI's HTML table. With Spring Data REST, the *_embedded* data set is considered a preview of data. While useful for glancing at data, to get headers like ETags, you need to fetch each resource individually. 
...
. The second `then(employeeCollection => ...)` clause converts the collection of employees into an array of GET promises to fetch each individual resource. *This is what you need to an ETag header for each employee.*
...
+
image::conditional/images/conditional-1.png[]
+
...
+
image::conditional/images/conditional-2.png[]

image::conditional/images/conditional-3.png[]
